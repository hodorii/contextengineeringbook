...
# OpenAI API 키 설정
os.environ["GOOGLE_API_KEY"] = os.getenv("GEMINI_API_KEY", "YOUR_GEMINI_KEY")
...
conversation_with_summary.invoke(
    {
        "input": "안녕? 내 이름은 이서준이야. 나는 AI 기술을 활용한 챗봇 개발에 관심이 많아."
    }
)
...
response = conversation_with_summary.invoke(
    {"input": "그 기술을 우리 회사 고객 지원팀에 어떻게 적용할 수 있을까?"}
)
